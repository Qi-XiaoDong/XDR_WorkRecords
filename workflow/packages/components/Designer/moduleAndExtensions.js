...
export default function (settings) {
  ...
  settings.contextPadMode === "enhancement" &&
  ...
  if (settings.rendererMode === "rewrite") {
    ...
    options["bpmnRenderer"] = {
      ...(settings.customTheme || {}),
      useCurve: settings.useCurve,
    };
  }
  ...
  if (!Object.keys(moddle).length) {
    if (settings.processEngine === "activiti")
      moddle["activiti"] = activitiModdleDescriptors;
    if (settings.processEngine === "camunda")
      moddle["camunda"] = camundaModdleDescriptors;
    if (settings.processEngine === "flowable")
