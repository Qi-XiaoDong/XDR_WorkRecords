      ...
      <el-table border :data="listeners" style="width: 100%" height="200px">
        ...
        <el-table-column label="操作" width="140">
          <template slot-scope="{ row, $index }">
            <el-button type="text" @click="openListenerModel($index, row)"
              >编辑</el-button
            >
            <el-button type="text" @click="removeListener($index)"
              >移除</el-button
            >
          </template>
        </el-table-column>
...
import {
  listenerTypeOptions,
  scriptTypeOptions,
} from "@packages/preset-configuration/enumsOption";
import {
  ...
  methods: {
      ...
      const list = this._listenersRaw.map((item) => ({
        ...
        ...(item.script
          ? {
              script: {
                ...item.script,
                scriptType: getScriptType(item.script),
              },
            }
          : {}),
    ...
    updateListenerType(value) {
      ...
      this.newListener = {
        ...
        type: value,
        ...(value === "script"
          ? { script: this.newListener.script || {} }
          : {}),
      };
    },
    ...
    async openListenerModel(index, listenerData) {
      ...
      listenerData &&
        (this.newListener = JSON.parse(JSON.stringify(listenerData)));
